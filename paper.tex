...
The feature-weight vector assigned a weight to each o the features. The
...
confidence = 0.25, minimal gain = 0.1, minimal leaf size = 2.  The
...
obtained better results when we take in to consideration both accuracy and  $\kappa$ values.

While these are promising results further experiments are needed. Other experiments should focus on novice programmers, but that is left as future
work. 

Further experiments with more data and more representative examples for all  the
clasess should give researchers the hability to do stratified samples to balence
...
\cite{blikstein2011using}; the parameters proposed by this author include the size of the program and changes in the code. Other authors also propose treating the use of certain keys as features e.g., the delete key because is manly used for correcting sentences.
% Also a multi-modal approach is needed. Since all these need
