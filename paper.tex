...
system. The proposed method uses a data mining approach in data 
genearated from both keyboard and mouse dynamics of students as they 
work in programming assignments. The data set was generated
...
classification algorithms: Na\"ive Bayes, J-48, k-NN, and Artificial Neural
...
proposed method, and results are discussed in Section \ref{sec:res}  % There is no results section so far
...
shown schematically in Figure \ref{fig_process} . % This should be
                                % a reference and I'm not sure which
                                % one, if it's now Fig 1 or 2. - JJ
                                % Corrected -MG
Users log in to 
Protoboard  by creaing an account or by using their account credentials from the
...
execute thier code. What follows after these videos, are 13 programming
exercises that students need to solve in sequential order. Exercises are of
incremental difficulty. In Figure \ref{fig_editor}, % 3 is keystroke
...
the initial survey is shown in Table \ref{tab_results}. %Please use reference. Fig 4
...
with 0.5 probability and mutation of 1/number of features, and a minimum feature size of 5.
...
The selected classifiers where implemented using Rapid Miner, with the following patameters:
...
is no restriction on what has to be writen. % don't get this. What free text? A program? - JJ
...
only to keep the space neeed. this is only to keep the space neeed. this is  only to 
keep the space neeed. only to keep the space neeed. this is only to keep the space neeed. 
this is  only to only to keep the space neeed. this is only to keep the space neeed. 
this is  only to  keep the space neeed. only to keep the space neeed. this is only to
this is  only to 
